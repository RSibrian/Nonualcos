                        ...
                        <table id="datatables" class="table table-striped table-no-bordered table-hover" cellspacing="0" width="100%" style="width:100%";>
                            ...
                            @foreach ($empleados as $empleado)
                                <tr>
                                    ...
                                    <td>$&nbsp;{{number_format(round($empleado->salarioBruto,2), 2, '.', ',')}}</td>
                                    <td>{{$empleado->dias_trabajados}}</td>
                                    <td>$&nbsp;{{number_format(round($empleado->salario_ganado,2), 2, '.', ',')}}</td>
                                    <td>${{number_format(round($empleado->ISSS,2), 2, '.', ',')}}</td>
                                    <td>${{number_format(round($empleado->AFP_empleado,2), 2, '.', ',')}}</td>
                                    <td>${{number_format(round($empleado->descuento_renta,2), 2, '.', ',')}}</td>
                                    <td>$ {{number_format(round($empleado->descuento_tiempo,2), 2, '.', ',')}} </td>
                                    <td>$ {{number_format(round($empleado->total_descuentos,2), 2, '.', ',')}}</td>
                                    <td>$ {{number_format(round($empleado->liquido,2), 2, '.', ',')}}</td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
